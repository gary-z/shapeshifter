...
```bash
cargo build --release --locked --example measure
python3 benchmarks/run.py --binary target/release/examples/measure \
  --puzzles benchmarks/puzzles.jsonl --output /tmp/native-results.jsonl \
  --variant current
python3 benchmarks/summarize.py /tmp/native-results.jsonl
```

The Python runner uses all logical CPUs through `RAYON_NUM_THREADS` and kills
the process group at the deadline. The driver reports preparation, search,
visited nodes, and placements; the runner independently replays every solution.
Records include puzzle and binary SHA-256 hashes. Failures and late solutions
...
```bash
python3 benchmarks/summarize.py benchmarks/reference.jsonl
```
...
```bash
python3 benchmarks/compare.py --baseline /tmp/measure-baseline \
  --candidate /tmp/measure-candidate --puzzles benchmarks/puzzles.jsonl \
  --output /tmp/paired
python3 benchmarks/summarize.py /tmp/paired/results.jsonl
```
...
The `bench` CLI surveys generated levels or captured puzzle history. Its timeout
starts **after preparation**, so its results use a different deadline from the
...
```bash
/tmp/shapeshifter-browser/bin/python web/bench_browser.py benchmarks/puzzles.jsonl \
...
shared-memory build. For Firefox, add `--browser firefox --executable-path` with
the executable from the browser setup guide. Browser scheduling, CPU reporting,
and WASM code generation can affect results independently of native performance.
