...
export async function testMoveScript(browser, script, puzzle, { hostedUrl, hard }) {
    const scenarios = ['success', 'start-lost', 'changed-before-moves', 'solver-error', 'timeout-restart', 'timeout-win', 'timeout-stop',
            'timeout-changed', 'timeout-bad-form', 'timeout-restart-error', 'unexpected-page', 'unexpected-final',
            'network-error', 'refused', 'uncertain-response', 'changed-board', 'cancel-search', 'stop'];
    for (const scenario of scenarios) {
        // A dead proxy also blocks requests that might escape interception.
        const context = await browser.newContext({ proxy: {
            server: 'http://127.0.0.1:1',
            bypass: new URL(hostedUrl).hostname,
        } });
        ...
        await context.route('**/*', async route => {
            ...
            if (url.origin === new URL(hostedUrl).origin) {
        ...
        try {
            ...
            if (['success', 'cancel-search'].includes(scenario)) {
            ...
            if (['success', 'start-lost', 'timeout-restart', 'timeout-win'].includes(scenario)) {
                if (scenario === 'success') await assertPlacementProgress(puzzle.pieces.length);
                ...
                if (['stop', 'timeout-stop'].includes(scenario)) {
                    await assertPlacementProgress(timeout ? TIMEOUT_PUZZLE.pieces.length : puzzle.pieces.length);
                ...
                assert.equal(requests.length, ['changed-before-moves', 'solver-error', 'timeout-changed', 'cancel-search'].includes(scenario) ? 0
                ...
                if (control.error) {
                    ...
                    if (scenario.startsWith('unexpected-')) {
                        ...
                        assert.equal(debug.request.html, 'You came from the wrong place!');
                    } else if (scenario === 'uncertain-response') {
                        assert.equal(debug.request.status, 500);
                        assert.equal(debug.request.html, 'The move happened, but its response failed.');
                    } else {
    ...
    console.log('PASS console auto-solve: hosted workers, countdown, placement progress, replay, timeout forfeits and restart, live-state checks, copyable error details, cancellation, stop/resume');
}
